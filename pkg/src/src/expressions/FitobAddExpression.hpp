...
/*
 * FitobAddExpression.hpp
 *
 *  Created on: Feb 27, 2010
 *      Author: benk
 */

#ifndef FITOBADDEXPRESSION_HPP_
#define FITOBADDEXPRESSION_HPP_
...
namespace fitob{
  ...
  /** */
  class AddExpression: public fitob::ExpressionBasis {
  public:

	 /** */
	 AddExpression(ExpressionBasis* left , ExpressionBasis* right);
	...
	void eval(const std::vector<DVector>& globalCoordonates , DVector& resVector) const {
	    ...
	    for (unsigned int ii = 0 ; ii < resVector.size() ; ii++)
	    	resVector[ii] += left_res[ii];
	}


    inline string toString() const { return "AddExpression(" +
