...
/*
 * FitobGteExpression.hpp
 *
 *  Created on: Feb 27, 2010
 *      Author: benk
 */

#ifndef FITOBGTEEXPRESSION_HPP_
#define FITOBGTEEXPRESSION_HPP_
...
namespace fitob{
  ...
  /** */
  class GteExpression: public fitob::ExpressionBasis {
  public:

	 /** */
	  GteExpression(ExpressionBasis* left , ExpressionBasis* right);
	...
	inline double eval(const DVector& globalCoordonates) const {
		return (left_->eval(globalCoordonates) >= right_->eval(globalCoordonates))? 1.0 : 0.0;
	}
	...
	void eval(const std::vector<DVector>& globalCoordonates , DVector& resVector) const {
	    ...
	    for (unsigned int ii = 0 ; ii < resVector.size() ; ii++){
	    	resVector[ii] = ((left_res[ii]) >= resVector[ii])? 1.0 : 0.0;
	    }
	}

    inline string toString() const { return "GteExpression(" +
