...
/*
 * FitobMinExpression.hpp
 *
 *  Created on: Feb 27, 2010
 *      Author: benk
 */

#ifndef FITOBMINEXPRESSION_HPP_
#define FITOBMINEXPRESSION_HPP_
...
namespace fitob{
  ...
  /** */
  class MinExpression: public fitob::ExpressionBasis {
  public:

	 /** */
	  MinExpression(ExpressionBasis* left , ExpressionBasis* right);
	...
	inline double eval(const DVector& globalCoordonates) const {
		return fmin(left_->eval(globalCoordonates) , right_->eval(globalCoordonates));
	}
	...
	void eval(const std::vector<DVector>& globalCoordonates , DVector& resVector) const {
	    ...
	    for (unsigned int ii = 0 ; ii < resVector.size() ; ii++){
	    	resVector[ii] = fmin(left_res[ii] , resVector[ii]);
	    }
	}

    inline string toString() const { return "MinExpression(" +
